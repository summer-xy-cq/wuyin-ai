...
export const handler = async (event, context) => {
    ...
    try {
        ...
        const { faceImage, tongueImage } = body

        if (!faceImage || !tongueImage) {
            return {
                statusCode: 400,
                headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
                body: JSON.stringify({ error: 'Missing faceImage or tongueImage' })
            }
        }
...
3. 严禁输出JSON以外的任何内容`
        ...
        if (!siliconResponse.ok) {
            ...
            return {
                statusCode: 500,
                headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
                body: JSON.stringify({ error: `AI服务调用失败: ${siliconResponse.status}` })
            }
        }
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                return {
                    statusCode: 500,
                    headers: { 'Content-Type': 'application/json', ...CORS_HEADERS },
                    body: JSON.stringify({ error: 'AI返回格式解析失败' })
                }
            }
        }
